      ...
      {filteredData.map((item) => (
        <div key={item.id} className="rounded-lg shadow-lg">
          <div className="h-[70vh] w-full lg:w-[410px] lg:h-[60vh] ">
            ...
            />
            <button className="bg-[hsl(180,82%,35%)] relative top-[-35px] text-lg ml-[200px] px-2 h-[55px]  text-white font-bold rounded-lg">
              <FontAwesomeIcon icon={faCalendarAlt} /> {item.days_and_night}
            </button>

            <div className="card_details1 p-7">
              <div className="text-xl text-[hsl(180,82%,35%)] pb-1">
                {item.country}
              </div>
              ...
              <div className=" text-[hsl(180,82%,35%)] pt-2">
              ...
              <div className="mt-1 text-end">
                <Link href={`/homepage/${item.id}`}>
                  <button
                    className="px-5 py-3 bg-[hsl(180,82%,35%)] text-white rounded-lg"
                    onClick={() => `/homepage+${item.id}`}
                  >
                    {" "}
                    Booknow
                  </button>
