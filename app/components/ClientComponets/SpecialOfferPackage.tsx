...
export default function SpecialTourPackageCard() {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.NEXT_PUBLIC_BACKEND_DOMAIN}/package/packagedetails/byoffer`, {
      method: "GET",
      headers: {
          ...
          <div className=" w-full h-full outerbox-image rounded-md xl:h-[30.2vh]">
            <div className="flex flex-col md:flex-row lg:flex-row overflow-hidden ">
              <div className="outerbox-image rounded-t-md  w-full lg:w-3/5 ">
                ...
                <div className="h-0 relative top-[-280px] md:bottom-0 left-[-40%] md:top-[-90%]  md:left-[-35%] lg:top-[-270px] lg:left-[-30%]  xl:top-[-80%] xl:left-[-30%] text-white z-20 text-[20px] lg:text-[18px] xl:text-[20px] font-bold font-sans  pb-10 bg-[#FDC703] text-center -rotate-45"> <span className=" text-[20px] lg:text-[18px] xl:text-[25px] font-bold font-sans">{item.offer} </span>% OFF</div>
              </div>
            ...
            <div className="card_details1 px-7 pt-10 w-full">
              ...
              <div className=" text-[hsl(180,82%,35%)] text-[18px] " style={titleFontStyle}> <FontAwesomeIcon icon={faCalendarAlt} className="pr-2"/>{item.days_and_night}</div>
              <br></br>
